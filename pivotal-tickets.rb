...
module ToOutput
  def to_row( overrides = { })
    ...
    sprintf("#%07d - (%s) (%s) (%s) [%s] %s\n", 
            ...
            charlimit((overrides[:component]  || self.story_type|| ""),10).ljust(10, ' '), 
            charlimit((overrides[:owner]      || self.requested_by || ""),7).ljust(7, ' '), 
...
Story.find(:all, :params => {:project_id => opts[:pivotal_project_id]}).each do |story|
  y story if @debug
  puts story.to_row
end
